  ...
  <div className="overflow-hidden w-full relative">
    ...
    >
      {images.map((img, i) => (
        <div
          key={i}
          onClick={() => setSelectedIndex(i)}
          className="flex-shrink-0 flex items-center justify-center border border-gray-300 bg-white aspect-square rounded-md"
          style={{
            width: `calc((100% - 3 * 0.5rem) / 4)`, // 4 ảnh + 3 khoảng cách gap
          }}
        >
          <img
            src={img}
    ...
    <button
      ...
      className={`absolute top-1/2 -translate-y-1/2 
        left-[calc((100% - 3 * 0.5rem) / 8 + 0.25rem)]  /* 👈 canh giữa ảnh đầu */
        bg-black/50 text-white p-2 rounded-full shadow-md z-10 
    ...
    <button
      ...
      className={`absolute top-1/2 -translate-y-1/2 
        right-[calc((100% - 3 * 0.5rem) / 8 + 0.25rem)] /* 👈 canh giữa ảnh cuối */
        bg-black/50 text-white p-2 rounded-full shadow-md z-10 
        transition-opacity duration-200
