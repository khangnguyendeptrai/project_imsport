    ...
    return (
        <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="relative flex-1 flex justify-center items-center">
                ...
                {/* Hiệu ứng lens (chỉ hiện trên PC) */}
{showZoom && (
  <div
    className="hidden md:block absolute border border-gray-600 bg-gray-300/20 pointer-events-none"
    style={{
      width: `${lensSize}px`,
      height: `${lensSize}px`,
...
{/* Vùng zoom phóng to (chỉ hiện trên PC) */}
{showZoom && (
  <div
  ...
  <div className="overflow-hidden w-full relative">
    ...
    >
      {images.map((img, i) => (
        <div
          key={i}
          onClick={() => setSelectedIndex(i)}
          className="flex-shrink-0 flex items-center justify-center border border-gray-300 bg-white aspect-square  relative"
          style={{
            width: `calc((100% - 3 * 0.5rem) / 4)`, // 4 ảnh + 3 khoảng cách (gap-2)
          }}
        >
          <img
            src={img}
    ...
    {/* Left arrow (đặt tại góc trái hình đầu tiên) */}
    <button
      onClick={handleMobilePrev}
      disabled={mobileStart === 0}
      className={`absolute top-1/2 -translate-y-1/2 
        left-[2px] bg-black/50 text-white p-2  shadow-md z-10
        transition-opacity duration-200
        ${mobileStart === 0 ? "opacity-40 cursor-not-allowed" : "opacity-100"}`}
      style={{
        transform: "translate(10%, -50%)", // đè chéo một chút lên ảnh đầu
      }}
    >
      <ChevronLeftIcon className="w-4 h-4" />
    </button>

    {/* Right arrow (đặt tại góc phải hình cuối cùng) */}
    <button
      onClick={handleMobileNext}
      disabled={mobileStart + mobileVisible >= images.length}
      className={`absolute top-1/2 -translate-y-1/2 
        right-[2px] bg-black/50 text-white p-2  shadow-md z-10
        transition-opacity duration-200
        ${mobileStart + mobileVisible >= images.length
          ? "opacity-40 cursor-not-allowed"
          : "opacity-100"}`}
      style={{
        transform: "translate(-10%, -50%)", // đè chéo một chút lên ảnh cuối
      }}
    >
