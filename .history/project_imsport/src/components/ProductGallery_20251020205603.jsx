          ...
          >
            {images.map((img, i) => (
              <div
                key={i}
                onClick={() => setSelectedIndex(i)}
             className="flex-shrink-0 w-1/4 aspect-square border border-gray-300 flex items-center justify-center bg-white mx-1 rounded-md"

              >
                <img
                  src={img}
                  alt={`mobile-thumb-${i}`}
                  className="w-[85%] h-[70%] object-contain"
                />
              </div>
            ))}
          </motion.div>
