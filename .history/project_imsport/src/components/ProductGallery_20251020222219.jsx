            ...
            <div className="md:hidden relative w-full mt-4">
                ...
                <div className="overflow-hidden w-full">
                    <motion.div
                        ...
                        className="flex w-full gap-2 px-1" // 👈 thêm gap và padding
                    >
                        {images.map((img, i) => (
                            <div
                                key={i}
                                onClick={() => setSelectedIndex(i)}
                                className="flex-shrink-0 w-[23%] aspect-square border border-gray-300 flex items-center justify-center bg-white "
                            >
                                <img
                                    src={img}
                                    alt={`mobile-thumb-${i}`}
                                    className="w-[75%] h-[70%] object-contain"
                                />
                            </div>
                        ))}
                    </motion.div>
                ...
                {/* Left arrow overlay (luôn hiển thị, mờ khi disable) */}
                <button
                    onClick={handleMobilePrev}
                    disabled={mobileStart === 0}
                    className={`absolute left-1 top-1/2 -translate-y-1/2 bg-black/50 text-white p-2 z-10 
    transition-opacity duration-200 ${mobileStart === 0 ? "opacity-40 cursor-not-allowed" : "opacity-100"
                        }`}
                >
                    <ChevronLeftIcon className="w-4 h-4" />
                </button>

                {/* Right arrow overlay (luôn hiển thị, mờ khi disable) */}
                <button
                    onClick={handleMobileNext}
                    disabled={mobileStart + mobileVisible >= images.length}
                    className={`absolute right-2 top-1/2 -translate-y-1/2 bg-black/50 text-white p-2 z-10 
    transition-opacity duration-200 ${mobileStart + mobileVisible >= images.length
