...
const ProductInfo = ({ name, brand, code, price, sizes, highlights }) => {
  ...
  return (
    <div className="w-full md:w-[90%] text-gray-800">
      ...
      <p className="text-sm text-gray-600 border-b border-gray-200 pb-4 mb-5">
        Thương hiệu:{" "}
      ...
      <p className="text-black pb-4">
        Giá: <span className="text-2xl font-semibold">{price}</span>
      </p>
      ...
      <div className="pb-6 border-b border-gray-200 mb-6 flex items-center gap-4 flex-wrap">
        <div>
          ...
          <input
            ...
            className="w-20 h-10 border border-gray-300 rounded-full text-center focus:border-[#FF9800] outline-none"
          />
        </div>

        <button className="bg-[#FF9800] text-white px-8 py-3 rounded-full uppercase font-medium hover:bg-[#ffa733] transition-all">
          Thêm vào giỏ hàng
        </button>

        <button className="w-10 h-10 border border-gray-300 rounded-full flex items-center justify-center hover:border-[#FF9800] hover:text-[#FF9800] transition-all">
