  ...
  <div className="overflow-hidden w-full relative">
    ...
    >
      {images.map((img, i) => (
        <div
          key={i}
          onClick={() => setSelectedIndex(i)}
          className="flex-shrink-0 flex items-center justify-center border border-gray-300 bg-white aspect-square rounded-md relative"
          style={{
    ...
    <button
      ...
      className={`absolute top-1/2 -translate-y-1/2 
        left-[2px] bg-black/50 text-white p-2 rounded-full shadow-md z-10
        transition-opacity duration-200
        ${mobileStart === 0 ? "opacity-40 cursor-not-allowed" : "opacity-100"}`}
      style={{
        transform: "translate(-40%, -50%)", // đè chéo một chút lên ảnh đầu
      }}
    >
    ...
    <button
      ...
      className={`absolute top-1/2 -translate-y-1/2 
        right-[2px] bg-black/50 text-white p-2 rounded-full shadow-md z-10
        transition-opacity duration-200
        ${mobileStart + mobileVisible >= images.length
          ? "opacity-40 cursor-not-allowed"
          : "opacity-100"}`}
      style={{
        transform: "translate(40%, -50%)", // đè chéo một chút lên ảnh cuối
      }}
    >
